...
function initEventListeners() {
    ...
    // ИСПРАВЛЕНИЕ: меняем обработчик кнопки "Перейти к статистике" на странице подтверждения
    const viewStatsBtn = document.getElementById('view-admin-btn');
    if (viewStatsBtn) {
        viewStatsBtn.addEventListener('click', function() {
            console.log('Кнопка "Перейти к статистике" нажата');
            showUserStats(); // Показываем статистику пользователя, а не админ-панель
        });
    }
...
function showUserStats() {
    ...
    // Получаем все ответы для расчета популярных ответов
    const allAnswers = JSON.parse(localStorage.getItem('newYearSurveyAnswers') || '{}');
